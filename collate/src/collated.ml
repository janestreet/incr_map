...
module Parametrized = struct
  module Stable = struct
    ...
    module V1 = struct
      ...
      [@@deriving
        ...
        , bin_io
        , compare
        , fields ~getters ~iterators:(create, fold)
        , diff ~stable_version:1
        , stable_witness
        , compare
        , equal]
    ...
    module V2 = struct
      ...
      include (
      struct
        let of_v1 (v1 : ('k, 'v) V1.t) : ('k, 'v) t =
          { data = v1.data
          ; num_filtered_rows = v1.num_filtered_rows
          ; key_range = v1.key_range
          ; rank_range = v1.rank_range
          ; num_before_range = v1.num_before_range
          ; range_widened_by = 0, 0
          ; num_unfiltered_rows = v1.num_unfiltered_rows
          }
        ;;

        let to_v1 t =
          { V1.data = t.data
          ; num_filtered_rows = t.num_filtered_rows
          ; key_range = t.key_range
          ; rank_range = t.rank_range
          ; num_before_range = t.num_before_range
          ; num_unfiltered_rows = t.num_unfiltered_rows
          }
        ;;
      end :
      sig
  ...
  let of_stable_v1 (v1 : ('k, 'v) Stable.V1.t) : ('k, 'v) t = Stable.V2.of_v1 v1
  ...
  let iter = Stable.V2.iter
  let to_alist = Stable.V2.to_alist
  let to_opaque_map = Stable.V2.to_opaque_map
  let first = Stable.V2.first
  ...
  module Private = struct
    ...
    module Stable = struct
      module V1 = struct
        let create = Stable.V1.Fields.create
      end
    end
  end
...
module Stable = struct
  ...
  module V1 = struct
    ...
    module type Concrete =
      Collated_intf.Concrete with type ('k, 'v) parametrized = ('k, 'v) Stable.V1.t
    ...
    struct
      module Key = Key
      module Value = Value
      include Parametrized.Stable.V1

      type ('k, 'v) parametrized = ('k, 'v) t

      module T = struct
        type nonrec t = (Key.t, Value.t) t [@@deriving sexp, bin_io, compare, equal]
       ...
       doesn't support parametrized types *)
        ...
        let to_diffs (t : t) =
          ...
          Fields.fold
            ~init:[]
            ~data:(fun acc field ->
              wrap_map_update (Update.Map.to_diffs (get field)) @ acc)
            ~num_filtered_rows:(fun acc field ->
