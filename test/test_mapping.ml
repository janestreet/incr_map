...
module%test [@name "random tests on filter and non-filter mapping functions"] _ = struct
  ...
  *)
  ...
  let%test_unit "mapping_i' with map rand test: start with empty map, stabilize every 10 \
                 steps"
    =
    test_mapping_i
      Int.Map.empty
