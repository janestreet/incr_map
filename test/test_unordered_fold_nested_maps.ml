...
module%test _ = struct
  ...
  let update_and_test ~initial_map ~dump =
    ...
    stage (fun ~f ->
      ...
      let result_without_update = Incr.Observer.value_exn incr_without_update_observer in
    ...
    let f (update : Update.t) map =
      ...
      | Add_key { outer_key; inner_key; data } ->
        Map.update map outer_key ~f:(fun inner_map ->
          Map.set (Option.value inner_map ~default:String.Map.empty) ~key:inner_key ~data)
    in
