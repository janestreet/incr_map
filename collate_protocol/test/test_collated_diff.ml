...
let diff_int ~from ~to_ = Diffable.Optional_diff.return (to_ - from)
let apply_int v d = v + d
let diff_string ~from:_ ~to_ = Diffable.Optional_diff.return to_
let apply_string _ d = d

let get_diff diff_k diff_v ~from ~to_ =
  let opt = Collated.Diff.get diff_k diff_v ~from ~to_ in
