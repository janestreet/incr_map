...
let%expect_test _ =
  ...
  let observer =
    Incremental.observe
      (Incr_set.inter (Incremental.Var.watch s1_var) (Incremental.Var.watch s2_var))
  in
  ...
  stabilize_and_show ();
  [%expect {| () |}];
  Incremental.Var.set s2_var (Int.Set.of_list [ 1; 2 ]);
  stabilize_and_show ();
  [%expect {| (1 2) |}];
  Incremental.Var.set s1_var (Int.Set.of_list [ 1 ]);
  stabilize_and_show ();
  [%expect {| (1) |}];
